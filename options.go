...
// On supported platforms connections retrieved from the connection pool are checked for liveness before using them.
// If the check fails, the respective connection is marked as bad and the query retried with another connection.
// checkConnLiveness=false disables this liveness check of connections.
func CheckConnLiveness(checkConnLiveness bool) Option {
	return func(c *Config) {
		c.checkConnLiveness = checkConnLiveness
	}
}
